...
# Create filehandles
xhmm_file = 'XHMM_Results.xcnv'
temp_xhmm_file = 'temp.' + xhmm_file
codex_file = 'Project_Name_CODEX_frac.txt'
temp_codex_file = 'temp.' + codex_file
clamms_file = 'clamms.compiled.cnv.bed'
temp_clamms_file = 'temp.' + clamms_file
master_bed_file = 'ucsc.refgenes.final'
phenotype_file = 'Phenotypes.txt'
output_file = 'Crossed_Results.txt'
...
subprocess.call(["sed '1d' " + str(xhmm_file) + "| sed 's/^.\{15\}//g' | sed 's/\-N//g' | sed 's/\-/    /' | sed 's/\:/    /' | awk 'BEGIN {OFS=\"\t\"} {print $3, $4, $5, $1, $2, $12, $6}' | sed 's/^/chr/g' > temp." + str(xhmm_file)], shell=True)
xhmm_formatted = subprocess.Popen("bedtools intersect -wa -wb -a " + str(master_bed_file) + ' -b ' + str(temp_xhmm_file) + " | uniq | awk 'BEGIN {OFS=\"\t\"} {print $1,$6,$7,$8,$4,$9,$10,$11}' | sort | uniq", shell=True, stdout=subprocess.PIPE)   
...
subprocess.call(["sed '1d' " + str(codex_file) + "| sed 's/\-N//g' | sed 's/dup/DUP/' | sed 's/del/DEL/' | awk '!($7 ~ \"-\")' | awk 'BEGIN {OFS=\"\t\"} {print $2, $5, $6, $1, $4, $11, $12, $7}' | sed 's/^/chr/g' > temp." + str(codex_file)], shell=True)
codex_formatted = subprocess.Popen("bedtools intersect -wa -wb -a " + str(master_bed_file) + ' -b ' + str(temp_codex_file) + " | uniq | awk 'BEGIN {OFS=\"\t\"} {print $1,$6,$7,$8,$4,$9,$10,$11,$12}' | sort | uniq", shell=True, stdout=subprocess.PIPE)
...
subprocess.call(["sed 's/\-N//g' " + str(clamms_file) + "| awk 'BEGIN {OFS=\"\t\"} {print $1, $2, $3, $5, $6, $9, $10}' | sed 's/^/chr/g' > temp." + str(clamms_file)], shell=True)
clamms_formatted = subprocess.Popen("bedtools intersect -wa -wb -a " + str(master_bed_file) + ' -b ' + str(temp_clamms_file) + " | uniq | awk 'BEGIN {OFS=\"\t\"} {print $1,$6,$7,$8,$4,$9,$10,$11}' | sort | uniq", shell=True, stdout=subprocess.PIPE)
